...
#include "CmpEmiconMemManagerItf.h"
    ...
    {\
        ...
        { \
          ...
          if (pICmpEmiconLog == NULL && s_pfCMCreateInstance != NULL) \
        { \
            pIBase = (IBase *)s_pfCMCreateInstance(CLASSID_CCmpEmiconLog, &initResult); \
            if (pIBase != NULL) \
            { \
                pICmpEmiconLog = (ICmpEmiconLog *)pIBase->QueryInterface(pIBase, ITFID_ICmpEmiconLog, &initResult); \
                pIBase->Release(pIBase); \
            } \
        } \
    ...
    {\
        ...
        { \
          ...
          if (pICmpEmiconLog != NULL) \
        { \
            pIBase = (IBase *)pICmpEmiconLog->QueryInterface(pICmpEmiconLog, ITFID_IBase, &exitResult); \
            if (pIBase != NULL) \
            { \
                 pIBase->Release(pIBase); \
                 if (pIBase->Release(pIBase) == 0) /* The object will be deleted here! */ \
    ...
    {\
        ...
        if (ERR_OK == importResult ) importResult = GET_CmpEmiconMemManager_file_to_mem(0);\
          if (ERR_OK == importResult ) importResult = GET_CmpEmiconMemManager_mem_to_file(0);\
          if (ERR_OK == importResult ) importResult = GET_clr_sys_log(0);\
          if (ERR_OK == importResult ) importResult = GET_create_log_files(0);\
          if (ERR_OK == importResult ) importResult = GET_get_next_log_item(0);\
...
#define USE_STMT \
    ...
    USE_PlcShellSkip      \
    USE_PlcShellSetEof      \
    USE_get_next_log_item      \
    USE_create_log_files      \
    USE_clr_sys_log      \
    USE_CmpEmiconMemManager_mem_to_file      \
    USE_CmpEmiconMemManager_file_to_mem     
...
#define USEEXTERN_STMT \
    ...
    EXT_PlcShellSkip  \
    EXT_PlcShellSetEof  \
    EXT_get_next_log_item  \
    EXT_create_log_files  \
    EXT_clr_sys_log  \
    EXT_CmpEmiconMemManager_mem_to_file  \
    EXT_CmpEmiconMemManager_file_to_mem 
