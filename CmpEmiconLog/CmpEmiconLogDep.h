 ...
 *  </copyright>
 */
 #ifndef _CMPEMICONLOGDEP_H_
#define _CMPEMICONLOGDEP_H_

#define COMPONENT_NAME "CmpEmiconLog" COMPONENT_NAME_POSTFIX
#define COMPONENT_ID    ADDVENDORID(CMP_VENDORID, CMPID_CmpEmiconLog)
#define COMPONENT_NAME_UNQUOTED CmpEmiconLog
...
/**
 * \file CmpEmiconLogItf.h
 */
#include "CmpEmiconLogItf.h"
    ...
    {\
        ...
        if (ERR_OK == importResult ) TempResult = GET_CmpEmiconEmu_is_emu(CM_IMPORT_OPTIONAL_FUNCTION);\
          ...
          if (ERR_OK == importResult ) importResult = GET_get_sys_var_area(0);\
           \
        /* To make LINT happy */\
...
#ifndef CMPEMICONLOG_DISABLE_EXTREF
#define EXPORT_EXTREF_STMT \
        { (RTS_VOID_FCTPTR)write_ts_log, "write_ts_log", 0xBBC343F7, 0x03051114 },\
          { (RTS_VOID_FCTPTR)write_ti_log, "write_ti_log", 0xEED4B686, 0x03051114 },\
          { (RTS_VOID_FCTPTR)write_sys_log, "write_sys_log", 0x9D6173B8, 0x03051114 },\
          { (RTS_VOID_FCTPTR)get_next_log_item, "get_next_log_item", 0xDA44E7BB, 0x03051114 },\
          { (RTS_VOID_FCTPTR)create_log_files, "create_log_files", 0x5B82FD55, 0x03051114 },\
          { (RTS_VOID_FCTPTR)clr_sys_log, "clr_sys_log", 0x8B141454, 0x03051114 },\
          
#else
#define EXPORT_EXTREF_STMT
#endif
#ifndef CMPEMICONLOG_DISABLE_EXTREF2
#define EXPORT_EXTREF2_STMT \
                    
#else
#define EXPORT_EXTREF2_STMT
#endif
#if !defined(CMPEMICONLOG_DISABLE_CMPITF) && !defined(STATIC_LINK) && !defined(CPLUSPLUS) && !defined(CPLUSPLUS_ONLY)
...
#define USEEXTERN_STMT \
    ...
    #define ComponentEntry CmpEmiconLog__Entry
#endif


#ifdef CPLUSPLUS

class CCmpEmiconLog : public ICmpEmiconLog 
{
    public:
        CCmpEmiconLog() : hCmpEmiconLog(RTS_INVALID_HANDLE), iRefCount(0)
        {
        }
        virtual ~CCmpEmiconLog()
        {
        }
        ...
        virtual void* QueryInterface(IBase *pIBase, ITFID iid, RTS_RESULT *pResult)
        {
            void *pItf;
            if (iid == ITFID_IBase)
                pItf = dynamic_cast<IBase *>((ICmpEmiconLog *)this);            
            else if (iid == ITFID_ICmpEmiconLog)
                pItf = dynamic_cast<ICmpEmiconLog *>(this); 
            else
            {
                if (pResult != NULL)
    ...
    public:
        RTS_HANDLE hCmpEmiconLog;
        int iRefCount;
};

#endif /*CPLUSPLUS*/
#ifdef RTS_COMPACT_MICRO
/* CmpEmiconLog Declarations for uRTS */
...
#define USE_HOOK_FUNCTION	RTS_RESULT CmpEmiconLog_HookFunction(RTS_UI32 ulHook, RTS_UINTPTR ulParam1, RTS_UINTPTR ulParam2)\
{\
...
#define USE_EXPORT_EXTREF const CMP_EXT_FUNCTION_REF CmpEmiconLog_ExternalsTable[] = \
	{\
		EXPORT_EXTREF_STMT\
		EXPORT_EXTREF2_STMT\
